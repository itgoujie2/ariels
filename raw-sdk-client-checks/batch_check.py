#!/usr/bin/env python3
"""Batch-run the raw a2a-sdk client compatibility check across a target list.
...
lighter check (card resolution + one message via the raw, current,
official a2a-sdk client) run as a supplementary pass over the same target
list, not a replacement for the main probers' full skill-by-skill
conformance run.
"""
...
from raw_sdk_check import check  # noqa: E402
...
async def check_one(sem: asyncio.Semaphore, target: str) -> dict:
    async with sem:
        ...
        return {"target": target, "raw_sdk_client": result, "_elapsed_s": round(time.monotonic() - start, 1)}
...
async def run(targets: list[str], output_name: str) -> None:
    ...
    for i, coro in enumerate(asyncio.as_completed(tasks), 1):
        ...
        status = "REACHABLE" if result["raw_sdk_client"]["reachable"] else "UNREACHABLE"
...
if __name__ == "__main__":
    ...
    output_name = sys.argv[2] if len(sys.argv) > 2 else "raw_sdk_client_results"
