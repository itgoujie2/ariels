...
same as the golden probers' run_prober.py, but calling out through the
raw, current, official `a2a-sdk` client instead of our hand-rolled
`a2a_wire.py` transport -- see raw_sdk_prober.py's docstring for the
design rationale and PLAN.md §11a for why this layer exists at all.
...
test -- whether a real caller using the raw SDK can complete each skill --
goes entirely through the raw a2a-sdk client, including letting it do its
own (separate, sometimes differently-behaved) card resolution and
rejecting an agent outright if that fails.
"""
...
from raw_sdk_prober import probe_skill_raw_sdk, resolve_client  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "raw-a2a-sdk-client",
        }
        ...
        results = []
        for skill in skills:
            transcript = await probe_skill_raw_sdk(client, skill)
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="Raw a2a-sdk native client")
