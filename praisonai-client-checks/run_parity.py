...
PraisonAI's own `praisonai.endpoints.providers.a2a.A2AProvider` instead
of our hand-rolled `a2a_wire.py` transport -- see praisonai_prober.py's
docstring for the design rationale and PLAN.md §11a for why this layer
exists.

Usage: python run_parity.py <base_url>

Card discovery/skill enumeration uses our own tolerant a2a_wire.fetch_card()
purely to learn what skills exist and what to ask about. The actual
test -- whether a real caller using PraisonAI's client can complete
each skill -- goes entirely through `A2AProvider`, including its own
hardcoded `{base_url}/a2a` endpoint assumption (see module docstring).
"""
...
from praisonai_prober import _explain_layer_error, probe_skill_praisonai  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "praisonai-client",
        }

        results = []
        for skill in skills:
            transcript = await probe_skill_praisonai(provider, skill)
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="PraisonAI native client")
