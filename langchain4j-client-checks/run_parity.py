...
LangChain4j's own first-party A2A client (via the one-shot Java bridge
process) instead of our hand-rolled `a2a_wire.py` transport -- see
langchain4j_prober.py's docstring for the design rationale and
...
from langchain4j_prober import _explain_layer_error, probe_skill_langchain4j, resolve_bridge  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "langchain4j-client",
        }
        ...
        results = []
        for skill in skills:
            transcript = await probe_skill_langchain4j(base_url, skill)
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="LangChain4j native client")
