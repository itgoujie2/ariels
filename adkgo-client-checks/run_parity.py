...
Google ADK for Go's own first-party remote-agent client (via the Go
bridge process) instead of our hand-rolled `a2a_wire.py` transport --
see adkgo_prober.py's docstring for the design rationale and PLAN.md
§11a for why this layer exists.
...
from adkgo_prober import probe_skill_adkgo, resolve_bridge  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "adkgo-client",
        }

        results = []
        for skill in skills:
            # A fresh bridge process (and remote-agent runner) per skill,
            # matching a real caller starting a new conversation per task.
            bridge, resolve_error = await resolve_bridge(base_url)
            if bridge is None:
                ...
                continue
            try:
                transcript = await probe_skill_adkgo(bridge, skill)
            finally:
                await bridge.close()
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="ADK-Go native client")
