"""Reachability check using Google ADK for Go's own first-party A2A
client (via the Go bridge process) -- see adkgo_prober.py's docstring
...
from adkgo_prober import resolve_bridge
...
async def check(base_url: str) -> dict:
    ...
    try:
        result = await bridge.call({"cmd": "generate", "sessionId": str(uuid.uuid4()), "text": PROBE_TEXT})
    ...
    if raw is None:
        return {"reachable": False, "reply": None, "error": "adk-go bridge returned no a2a response"}
