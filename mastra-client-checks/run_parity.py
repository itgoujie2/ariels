...
Mastra's own first-party `@mastra/core/a2a` client (via the Node bridge
process) instead of our hand-rolled `a2a_wire.py` transport -- see
mastra_prober.py's docstring for the design rationale and PLAN.md §11a
for why this layer exists (the first cross-language one in the matrix).
...
from mastra_prober import _explain_layer_error, probe_skill_mastra, resolve_bridge  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "mastra-client",
        }

        results = []
        for skill in skills:
            # A fresh bridge process (and A2AAgent instance) per skill,
            # matching a real caller starting a new conversation per task.
            bridge, _card, resolve_error = await resolve_bridge(base_url)
            if bridge is None:
                results.append(assertions.summarize(skill["id"], {
                ...
                continue
            try:
                transcript = await probe_skill_mastra(bridge, skill)
            finally:
                await bridge.close()
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="Mastra native client")
