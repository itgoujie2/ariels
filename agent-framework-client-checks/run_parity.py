...
Microsoft's own first-party `agent_framework_a2a.A2AAgent` instead of
our hand-rolled `a2a_wire.py` transport -- see
agent_framework_prober.py's docstring for the design rationale and
PLAN.md §11a for why this layer exists.
...
from agent_framework_prober import _explain_layer_error, probe_skill_agent_framework, resolve_agent  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "agent-framework-client",
        }
        ...
        results = []
        for skill in skills:
            transcript = await probe_skill_agent_framework(agent, skill)
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="Microsoft Agent Framework native client")
