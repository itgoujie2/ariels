#!/usr/bin/env python3
"""Batch-run the agent-framework A2AAgent compatibility check across a target list.
...
from agent_framework_check import check  # noqa: E402
...
async def check_one(sem: asyncio.Semaphore, target: str) -> dict:
    async with sem:
        ...
        return {"target": target, "agent_framework_client": result, "_elapsed_s": round(time.monotonic() - start, 1)}
...
async def run(targets: list[str], output_name: str) -> None:
    ...
    for i, coro in enumerate(asyncio.as_completed(tasks), 1):
        ...
        status = "REACHABLE" if result["agent_framework_client"]["reachable"] else "UNREACHABLE"
...
if __name__ == "__main__":
    ...
    output_name = sys.argv[2] if len(sys.argv) > 2 else "agent_framework_client_results"
