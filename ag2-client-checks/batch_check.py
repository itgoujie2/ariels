#!/usr/bin/env python3
"""Batch-run the AG2 client compatibility check across a target list.
...
lighter check (card resolution + one message via AG2's real, official
A2aRemoteAgent) run as a supplementary pass over the same target list,
not a replacement for the main probers' full skill-by-skill conformance
run.
"""
...
from ag2_check import check  # noqa: E402
...
async def check_one(sem: asyncio.Semaphore, target: str) -> dict:
    async with sem:
        ...
        return {"target": target, "ag2_client": result, "_elapsed_s": round(time.monotonic() - start, 1)}
...
async def run(targets: list[str], output_name: str) -> None:
    ...
    for i, coro in enumerate(asyncio.as_completed(tasks), 1):
        ...
        status = "REACHABLE" if result["ag2_client"]["reachable"] else "UNREACHABLE"
...
if __name__ == "__main__":
    ...
    output_name = sys.argv[2] if len(sys.argv) > 2 else "ag2_client_results"
