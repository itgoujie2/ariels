...
Agno's own first-party `agno.client.a2a.client.A2AClient` instead of
our hand-rolled `a2a_wire.py` transport -- see agno_prober.py's
...
from agno_prober import _explain_layer_error, probe_skill_agno, resolve_client  # noqa: E402
...
async def main(base_url: str) -> dict:
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as httpx_client:
        ...
        report: dict = {
            ...
            "prober_implementation": "agno-client",
        }
        ...
        results = []
        for skill in skills:
            transcript = await probe_skill_agno(client, httpx_client, skill)
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    html = report_render.render(result, engine_label="Agno native client")
