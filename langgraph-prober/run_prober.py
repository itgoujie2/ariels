...
It never assumes a domain. It reads the target's own agent card (name,
skills, examples) to decide what to test, asks the target directly what it
does (a live conversational cross-check against the card), then drives each
declared skill to a stopping point and reports deterministic protocol-
conformance results per skill.
...
from app.prober import TurnState, graph  # noqa: E402
...
def _write_run(base_url: str, result: dict) -> Path:
    ...
    # customer running this open-source prober directly, with no hosted
    # product involved, should still get something to actually look at.
    html = report_render.render(result, engine_label="LangGraph golden prober")
