...
from app.a2a_wire import UnrecognizedResponseShapeError, _normalize, classify_error_text, explain_error, resolve_target
...
# -- classify_error_text() / explain_error() --
#
# Shared across every prober and native-client layer (see a2a_wire.py's own
# module-level comment for why this needs to be text-based, not
# isinstance-based like version_probe.py's own _classify_error()).


def test_classify_error_text_unsupported():
    assert classify_error_text('A2A error -32601: Method not found: "SendMessage"') == "unsupported"
    assert classify_error_text("RuntimeError: Method not found") == "unsupported"


def test_classify_error_text_auth_required():
    assert classify_error_text("Authentication required") == "auth_required"
    assert classify_error_text("Client error '401 Unauthorized' for url ...") == "auth_required"
    assert classify_error_text("403 Forbidden") == "auth_required"


def test_classify_error_text_internal_error_wrapping_a_nested_401_is_not_misclassified_as_auth():
    """Regression: real finding, neva.dt-agent.co.uk -- a -32603 'Internal
    error' can legitimately leak an upstream 401 INSIDE its own wrapped
    message text without meaning the caller itself needs to authenticate
    (the target's own backend integration is what's broken). -32603 must
    be checked before the looser 401/403 substring match, or this
    real case gets misclassified as auth_required."""
    raw = (
        "A2A error -32603: Error code: 401 - {'type': 'error', 'error': "
        "{'type': 'authentication_error', 'message': 'invalid x-api-key'}}"
    )
    assert classify_error_text(raw) == "internal_error"


def test_classify_error_text_payment_required():
    assert classify_error_text("HTTP 402 Payment Required") == "payment_required"
    assert classify_error_text("this endpoint requires payment via x402") == "payment_required"


def test_classify_error_text_internal_error():
    assert classify_error_text("A2A error -32603: Internal error") == "internal_error"


def test_classify_error_text_malformed_response_is_other_not_unsupported():
    """Same reasoning as version_probe.py's own identical check: a non-JSON
    or non-JSON-RPC body means the endpoint is broken/wrong, not that the
    dialect specifically isn't understood."""
    assert classify_error_text("Response body was not valid JSON: ...") == "other"
    assert classify_error_text("JSON-RPC response has neither 'result' nor 'error'") == "other"


def test_classify_error_text_unrecognized_falls_back_to_other():
    assert classify_error_text("connection refused") == "other"


def test_explain_error_wraps_known_buckets_and_preserves_raw_text():
    raw = 'RuntimeError: A2A error -32601: Method not found: "SendMessage". This agent supports "message/send".'
    explained = explain_error(raw)
...
def test_explain_error_passes_through_unclassified_errors_unchanged():
    """An error shape this project hasn't seen before must never be
    dropped or garbled -- falls back to the raw text as-is."""
    raw = "some brand-new error text never documented before"
    assert explain_error(raw) == raw
